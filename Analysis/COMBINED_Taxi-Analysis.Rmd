...
# Introduction
ONE STATEMENT WHAT THE NOTEBOOK IS ABOUT
This r markdown file contains our code we used in our EDA. Our objective was to identify what factors contribute to tipping amount for taxicab services.
...
boxplot(unprocessed_data$tip_amount)
...
```{r Outlier detection and missing values, echo=F, include=T}
# fare amount in negative & credit card payments & trip_distance > 0 & VendorID == 4
processed_df <- unprocessed_data %>% filter((fare_amount > 0) & (payment_type == 1) & (passenger_count < 7) & (trip_distance > 0)  & (VendorID != 4))

hist(processed_df$tip_amount)
...
```{r merge location descriptions, include = T}
...
processed_df$pickup_hrs <- as.numeric(processed_df$pickup_hrs)
processed_df$pickup_period <- ifelse(processed_df$pickup_hrs >= 05 & processed_df$pickup_hrs <= 11, "Morning",
            ifelse(processed_df$pickup_hrs > 11 & processed_df$pickup_hrs <= 16, "Afternoon",
            ifelse(processed_df$pickup_hrs > 16 & processed_df$pickup_hrs <= 21, "Evening", "Night")))
processed_df$pickup_period <- as.factor(processed_df$pickup_period)
processed_df$pickup_period <- ordered(processed_df$pickup_period, levels = c("Morning", "Afternoon", "Evening", "Night"))
```
...
processed_df$dropoff_hrs <- as.numeric(processed_df$dropoff_hrs)
processed_df$drop_period <- ifelse(processed_df$dropoff_hrs >= 05 & processed_df$dropoff_hrs <= 11, "Morning",
            ifelse(processed_df$dropoff_hrs > 11 & processed_df$dropoff_hrs <= 16, "Afternoon",
            ifelse(processed_df$dropoff_hrs > 16 & processed_df$dropoff_hrs <= 21, "Evening", "Night")))
processed_df$drop_period <- as.factor(processed_df$drop_period)
processed_df$drop_period <- ordered(processed_df$drop_period, levels = c("Morning", "Afternoon", "Evening", "Night"))
```
...
#Trip_duration
processed_df$trip_duration <- difftime(processed_df$dropoff_datetime, processed_df$pickup_datetime, units = "mins")
processed_df$trip_duration <- floor(as.numeric(processed_df$trip_duration))
boxplot(processed_df$trip_duration, col = c("#0000FF"))

duration_outliers <- boxplot(processed_df$trip_duration, col = c("#0000FF"))

processed_df = processed_df[!processed_df$trip_duration %in% duration_outliers$out, ]

boxplot(processed_df$trip_duration, col = c("#0000FF"))
min(processed_df$trip_duration)
max(processed_df$trip_duration)
```
...
## GEOGRAPHIC LOCATION AND TIP (Steven)
...
# ?? question remains whether I factor on basis of mean or median
...
# Here print a histogram for per_tip to show that it is more normalized.

# Now that I know my variables, which test to choose. I cannot use z-test because I dont know population mean & std dev. I also cannot use one sample t-test because I dont have a pre-determined population mean or some other theoretically derived value with which i could compare the mean value of my observed sample.  

# Bimodal so divided it into 2 chunks for normalization

# So going to use independent two-sample t-test, a significance test that can give us an estimate as to whether different means between two groups are the result of random variation or the product of specific characteristics within the groups. For that I divide my independent variable i.e. distances covered in miles during each ride into "two factored categorical data" Short & Long. I divided this column into 2 factors based on mean value of distance travelled.
...
p <- ggplot(df_dist, aes(x=df_dist$trip_distance, y=df_dist$tip_amount)) + 
  geom_boxplot() + stat_summary(fun.y=mean, geom="point", shape=23, size=4)  # adding mean to box plot
p

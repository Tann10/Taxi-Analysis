...
ggplot(data = processed_df, aes(x=factor(0), per_tip)) + 
...
```{r, echo=F, include=T}
#ggplot histogram of per_tip for processed df
processed_df %>%
  ggplot(aes(per_tip)) +
  geom_histogram(aes(y =..density..),  colour = "black", fill = "#66B2FF") + 
  stat_function(fun = dnorm, args = list(mean = mean(processed_df$per_tip), sd = sd(processed_df$per_tip))) + ggtitle("Distribution of NYC Taxi Tip Data Post-Outlier Removal")
```
**Observation :** 
...
We hyothesis that there is a relationship between the number of passenger in the car to the tip amount paid to the driver. The passenger count varies from 0 to 6.  
...
processed_df %>%
  ggplot(aes(passenger_count, per_tip, color = passenger_count)) +
  geom_boxplot() +
  theme(legend.position = "none") +
  labs(y = "Percentage tip amount [$]", x = "Number of passengers")
```
...
processed_df %>%
  ggplot(aes(VendorID, per_tip, color = VendorID)) +
  geom_boxplot() +
  theme(legend.position = "none") +
  labs(y = "Percentage tip amount [$]", x = "Vendor ID")
...
```{r anova test for vendor ID, echo=T, include=T}
# anova tests
anova_vendor = aov(per_tip ~ VendorID, data = processed_df)
summary(anova_vendor)

```
The P value `r summary(anova_tip_amount)[[1]][["Pr(>F)"]][1]` is > 0.05, hence we fail to reject null hypothesis. 

Hence, we can conclude that the vendor does not affect tip amount
...
ggplot(data = processed_df, aes(x = pickup_period, fill = pickup_period)) + geom_bar() + ggtitle("Trip Counts Based on Pickup Time") + xlab("Time Period") + ylab("Tip Percentage")

# Plot tip percentage by Pickup_Time
ggplot(data = processed_df, aes(x = pickup_period, y = per_tip, fill = pickup_period)) + ggtitle("Tip Percentage by Pickup Time Period") + geom_boxplot() + xlab("Time Period") + ylab("Tip Percentage")
...
ggplot(data = processed_df, aes(x = drop_period, fill = drop_period)) + geom_bar() + ggtitle("Trip Counts Based on Dropoff Time") + xlab("Time Period") + ylab("Tip Percentage")

# Plot tip percentage by Dropoff_Time
ggplot(data = processed_df, aes(x = drop_period, y = per_tip, fill = drop_period)) + ggtitle("Tip Percentage by Dropoff Time Period") + geom_boxplot() + xlab("Time Period") + ylab("Tip Percentage")
...
#plot a tip paid vs trip duration histogram
hist(processed_df$trip_duration, processed_df$per_tip, main = "Histogram of Trip_Duration vs Tip_Percentage", col = rainbow(40), breaks = 40, xlab = "Trip_Duration", ylab = "Tip_Percentage" )
```
...
a <- summary(processed_df$trip_distance)
a
# str(processed_df)
names(a)
```
First we take a look at summary of Trip Distance column... It shows that min is `r a[['Min.']]`,  max is `r a[['Max.']]` and mean is `r a[['Mean']]`, however are closer to min as compared to max which indicates that we have some outliers in distance data but we should not worry because it is our independent variable.
...
# subsetting data on basis for distance travelled for t-test
...
result_t <- t.test(short_dist$per_tip, long_dist$per_tip)
print(result_t)

```
With Significance level set to 5%, I get a P-value of `r result_t$p.value` that is very close to zero with confidence interval level of `r result_t$conf.int`. Concluding that I have enough evidence to reject the Null hypothesis in favor of Alternate Hypothesis meaning that people travelling through Yellow cabs in NYC tip differently based on distance travelled.

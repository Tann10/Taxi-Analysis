...
```{r summary of the unprocessed data, echo=F, include=T}
...
```{r Outlier detection and missing values, echo=F}

# fare amount in negative & credit card payments & trip_distance > 0
processed_df <- unprocessed_data %>% filter((fare_amount > 0) & (payment_type == 1) & (passenger_count < 7) & (trip_distance > 0) )
...
processed_df$pickup_datetime <- as.POSIXlt(as.character(processed_df$tpep_pickup_datetime), format="%Y-%m-%d %H:%M:%S")
processed_df$pickup_time <- format(processed_df$pickup_datetime, format = "%H:%M:%S")
processed_df$pickup_hrs <- format(strptime(processed_df$pickup_time,"%H:%M:%S"),'%H')
...
processed_df$dropoff_datetime <- as.POSIXlt(as.character(processed_df$tpep_dropoff_datetime), format="%Y-%m-%d %H:%M:%S")
processed_df$dropoff_time <- format(processed_df$dropoff_datetime, format = "%H:%M:%S")
processed_df$dropoff_hrs <- format(strptime(processed_df$dropoff_time,"%H:%M:%S"),'%H')
...
#head(processed_df)
#min(processed_df$trip_duration_mins1)
#max(processed_df$trip_duration_mins1)
#subset(processed_df, trip_duration_mins1 == 1439)
#nrow(processed_df)
...
### Tip payed and distance travelled by passengers 

```{r distance_vs_tip, inlcude=T, echo=F}
# AMNA
...
<!-- plot(processed_df$trip_distance, processed_df$tip_amount, main = "Scatter plot between Percentage Tip Amount & Distance travelled (Log)", log = processed_df$tip_amount) -->
...
<!-- shapiro.test(processed_df$trip_distance) -->
<!-- str(processed_df) -->

mean_dist <- mean(processed_df$trip_distance)
# should i use mean or median bc this col in not normal

factor_dist <- cut(processed_df$trip_distance, breaks=c(0, mean_dist, Inf), labels=paste("Distance", 1:2, sep=""))
# Factoring distance col

processed_df$trip_distance <- factor_dist
# assigning factored col to the original df
str(processed_df)
...
<!-- median_dist <- median(processed_df$trip_distance) -->

<!-- factor_dist <- cut(processed_df$trip_distance, breaks=c(0, median_dist, Inf), labels=paste("Distance", 1:2, sep="")) -->
<!-- # Factoring distance col -->
...
<!-- plot(processed_df$per_tip ~ processed_df$trip_distance, data=processed_df , main="Tips paid by Short vs High Distance Commuters (Median)", ylab="Tips Paid", xlab="Distance travelled", col=c("#ff0000","#00ff00") ) -->
<!-- M = tapply(processed_df$per_tip, -->
<!--            INDEX = processed_df$trip_distance, -->
<!--            FUN   = median) -->
<!-- points(M, -->
...
<!-- # same results as those for mean, above plot shows that mean tip paid by short-distance travellers is greater than long-distance travellers -->

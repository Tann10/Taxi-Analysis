...
# Get summary of data
summaryy <- data.frame("Dtype" = sapply(unprocessed_data,typeof),"MIN" = apply(unprocessed_data,2,min), "MEDIAN" = apply(unprocessed_data,2,median),  "MEAN" = apply(unprocessed_data,2,mean), "MAX" = apply(unprocessed_data,2,max))
...
# Create qqplot to check normality
...
summary_after <- data.frame("Dtype" = sapply(processed_df,typeof), "MIN" = apply(processed_df,2,min), "MEDIAN" = apply(processed_df,2,median),  "MEAN" = apply(processed_df,2,mean), "MAX" = apply(processed_df,2,max))
kable(summary_after)
...
After checking normality and treating outlier the scope of the initial question was changed to *analysing if factors such as location, duration, passenger count and time of the day affect to the ratio of the tip amount and the fare amount paid by credit card*
...
In this section we analyze various factors which may affect the tipping amount paid to the cab drivers using a subset of yellow taxi New York City data pertaining to June 2019.
...
```{r box_plot_dist, include = T, echo = T}
# get summary
...
We hyothesize that there is a relationship between the number of passenger in the car and  to the tip amount paid to the driver. The passenger count varies from 0 to 6.
...
processed_df %>%
  ...
  ggtitle("Box plot distribution for tip ratio and passenger count")
```

```{r anova test for passenger_count, echo=F,  include=F}
...
```
The mean of all the groups of passenger count is not varying much. Also the results of anova gives the P value `r summary(anova_tip_amount)[[1]][["Pr(>F)"]][1]` which is > 0.05, hence we fail to reject null hypothesis. This means that the number of passenger in the car does not affect ratio of the tip amount.

```{r t test for vendor ID, echo=F, include=F}
# Create factor
...
Similarly we check the distribution for the vendor ID and run t test to check the results. The P value `r ttest_vendor$p.value` is > 0.05, hence we fail to reject null hypothesis and conclude that the vendor does affect tip amount
...
trip duration | T-test | `r format(trip_duration_ttest$p.value, digits=3)` | means are equal | reject H0 |
passenger count | ANOVA | `r summary(anova_tip_amount)[[1]][["Pr(>F)"]][1]` | means are equal | fail to reject H0 |
vendor ID | T-test | `r ttest_vendor$p.value` | means are not equal | reject H0 | 

*based on based on a significance level of 0.05
...
Lynn, W. M. (2004). Tipping in restaurants and around the globe: An interdisciplinary review. <i>Handbook of Contemporary Behavioral Economics: Foundations and Developments.</i> Retrieved from https://ssrn.com/abstract=465942.
...
Zhan, X., Hasan, S., Ukkusuri, S., & Kamga, C. (2013). Urban link travel time estimation using large-scale taxi data with partial information. <i>Transportation Research Part C, 33</i>, 37–49. doi: 10.1016/j.trc.2013.04.001

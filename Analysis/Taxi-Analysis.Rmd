...
Before preprocessing the number of rows and columns are : `r nrow(unprocessed_data)`, `r ncol(unprocessed_data)`
...
The columns which are not used for analysis can be deleted.
```{r Drop_columns }
...
After preprocessing the number of rows and columns are : `r nrow(processed_df)`, `r ncol(processed_df)`
...
# Create boxplot
processed_df %>%
  ggplot(aes(passenger_count, per_tip, fill = passenger_count)) +
  geom_boxplot() +
  theme(legend.position = "none") +
  labs(y = "Percentage tip amount [$]", x = "Number of passengers") + 
  ggtitle("Box plot distribution for % tip amount and passenger count")
```
...
processed_df %>%
  ggplot(aes(VendorID, per_tip, fill = VendorID)) +
  geom_boxplot() +
  theme(legend.position = "none") +
  labs(y = "Percentage tip amount [$]", x = "Vendor ID") + 
  ggtitle("Box plot distribution for % tip amount and vendor")
```
...
# t test for vendor
ttest_vendor = t.test(per_tip ~ VendorID, data = processed_df)
ttest_vendor
```
...
#plot a frequency of Tip Ratio vs trip duration histogram
ggplot(data = processed_df, aes(x=trip_duration)) + geom_histogram(stat="bin", position = "stack",binwidth = 0.5, col="red", aes(fill=..count..))+ ggtitle("Tip Ratio Based on Duration") + xlab("Trip Frequency") +

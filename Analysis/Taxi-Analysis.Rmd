...
**Observation**: The p-values are `r summary(anova_tiprat_pu)[[1]][["Pr(>F)"]][[1]]` and `r summary(anova_tiprat_do)[[1]][["Pr(>F)"]][[1]]` for pick up and drop off, respectively. Both of these are smaller than a significance level of 0.05 (a 0.95 confidence level). Thus, we can reject the null hypothesis that the means are the same and say the means are statistically different at a significance level of 0.05.
...
# Plot Tip ratio vs Dropoff_Time

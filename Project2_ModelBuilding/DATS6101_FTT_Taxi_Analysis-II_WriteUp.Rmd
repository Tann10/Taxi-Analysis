...
Non-tipping work related to the NYC dataset have been conducted, especially in regard to pick up and drop off locations (Zhan, Hasan, Ukkusuri, & Kamga, 2013; Neutens, Delafontaine, Scott, & De Maeyer, 2012) and to supply and demand (Qian & Ukkusuri, 2015; Gonzales, Yang, Morgul, & Ozbay 2014). Accessibility/location (Qian & Ukkusuri, 2015), income, population, age, and number of jobs were strongly related to demand (Qian & Ukkusuri, 2015; Gonzales, Yang, Morgul, & Ozbay 2014). Sun and McIntosh (2016) discover that taxicab ridership is lowest in the early mornings and the highest in the evenings. They also note that for short distances, trip time, distance, and fare are all linearly related; however, this relationship grows weaker at greater distances due to the influence of other potential factors.
...
The source dataset used for the project was taken from the New York City Taxi and Limousine Commission (2019) website (https://www1.nyc.gov/site/tlc/about/tlc-trip-record-data.page).
...
As always, first step in every significance testing is to design hypothesis. Below are the hypothesis for tip duration: 
<ul>
...
With Significance level set to 5%, we got a P-value very close to zero. Concluding that we have enough evidence to reject the Null Hypothesis in favor of Alternate Hypothesis meaning that people travelling through yellow cabs in NYC tip differently based on distance travelled.


### Trip duration

We hyothesize that there is a relationship between the Duration of the trip and the tip fare ratio. Trip Duration is calculated by subtracting pickup time from drop time. Trip duration is the number of minutes taked for the trip.

For this test our hypothesis were as below:
<ul>
...
From the results we observe that p-value `r format(td_ttest$p.value, digits = 3)` is < 0.05 With Significance level set to 5% hence we reject null hypothesis.
...
We hyothesize that there is a relationship between the number of passenger in the car and the tip fare ratio. We are considering passenger count as a cateogrical variables. There are 6 factors of passenger count varing from 1 to 6.
...
```
The mean of all the groups of passenger count does not vary much. Also, the output of ANOVA gives the p-value `r format(summary(anova_tip_amount)[[1]][["Pr(>F)"]][1], digits=3)` which is < 0.05, hence we reject null hypothesis. This means that the number of passenger in the car does affect ratio of the tip amount.
...
Since our data problem falls under regression we chose decision trees and linear(uni,multivariate) regression models and find the best one that suits our data. We also applied step wise feature selection, Lasso, Ridge and Elastic Net and PCA to improve our linear model. Let us look into them in details below.

## Model Building Preparation

### One-hot encoding 
In order to convert factor columns to numerical columns we have used one-hot encoding technique. One-hot encoding converts all factors into separate boolean columns. 
...
After one-hot encoding we get total `r dim(com_df_sub)[2]` columns. 
...
Now that our EDA is complete with Tip Fare Ratio as dependent variable, the next step is to apply a variety of regression based algorithms allowing us to extract insights from data that we can then use to tell which outcome is likely to hold true for our target variable based on training data. 

Model evaluation metric used for all models is r2 and mean absolute percentage error (MAPE). MAE or MSE are scale dependent variables. We wanted to use scale independent model evaluation creteria hence we decided to use MAPE.MAPE is measured as follows:
![MAPE formula](MAPE.PNG)


### Linear Regression

We built linear model with tip amount and first 3 highly correlated values for 'Tip_Amount' those are fare_amount (-0.31), trip_distance (-0.27), and trip_duration(-0.23).
...
fit1 <- lm(tip_fare_ratio ~ trip_duration, data = taxipro )
#summary(fit1)
# vif(fit1)
# plot(fit1)
...
fit2 <- lm(tip_fare_ratio ~ trip_duration+fare_amount, data = taxipro )
#summary(fit2)
# vif(fit2)
# plot(fit2)
...
fit3 <- lm(tip_fare_ratio ~ trip_duration+fare_amount+trip_distance, data = taxipro )
#summary(fit3)
#vif(fit3)
#plot(fit3)
```
...
Feature (variable)  |  R^2  | 
-----|-----|  
lm(tip_fare_ratio ~ trip_duration) |`r format(summary(fit1)$r.squared, digits=3)`
lm(tip_fare_ratio ~ trip_duration+fare_amount) |`r format(summary(fit2)$r.squared, digits=3)`
lm(tip_fare_ratio ~ trip_duration+fare_amount+trip_distance) |`r format(summary(fit3)$r.squared, digits=3)`

Let us perform ANOVA test on all the three models. 
...
```
After removal of the outliers from the data there is no improvement in the values for tip fare ratio. Below are the values for MAPE and R2.
```{r }
results_df
```
...
Below is the tip_fare_ratio ~ . step  feature selection with Adjusted R^2 scaling
...
From the graph above we see that trip_duration, congestion_surcharge, Borough_do_Unknown are the three features that give maximum Adj R^2 = 0.11 after which Adj R^2 remians constant.
...
To further improve the model Ridge, Lasso, Elastic Net, and PCA is performed as follows.
...
To choose the best lambda though we should consult the MSE vs lambda plot as given below. The best lambda value in our case turns out to be `r cv_lamda$lambda.min`. 

```{r ridge2plt, include = T, echo = F}
plot(cv_lamda)
```

But R-squared obtained (i.e. the amount of variance that our model can account for) is only 12% which means there is room for a lot improvement.
...
But R-squared obtained (i.e. the amount of variance that our model can account for) is only 12% (as shown in graph below) which means there is room for a lot improvement.
...
We built linear model with tip amount and first 3 highly correlated values for 'Tip_Amount' those are fare_amount(0.91), trip_distance(0.83), and trip_duration(0.71).
...
fit2_ta <- lm(tip_amount ~ fare_amount+trip_distance, data = taxipro )
#summary(fit2_ta)
# vif(fit2_ta)
# plot(fit2_ta)
...
Feature (variable)  |   R^2  | 
-----|-----|  
lm(tip_amount ~ fare_amount) |`r format(summary(fit1_ta)$r.squared, digits=3)`
lm(tip_amount ~ fare_amount+trip_distance) |`r format(summary(fit2_ta)$r.squared, digits=3)`
lm(tip_amount ~ fare_amount+trip_distance+trip_duration) |`r format(summary(fit3_ta)$r.squared, digits=3)`
...
The results are good. In order to check if any combination of features set give us better results than this we performed Stepwise Regression.

### Stepwise regression

We performed backward stepwise regression and variables were selected based on adjusted R2
...
From the graph above we see that Adj R^2 remains constant throughout. Hence we select fare_amount, congestion_surcharge, Borough_do_Bronxn as our first three features that give maximum Adj R^2 = 0.83.

Below is the Summary from Lm with First 3 features selected with Feature Selection.
...
So replacing our dependent variabe to tip amount is giving us good R-square.  Lets see if we get the the same results from Ridge Model too. We perform all the steps exactly in same manner as we did earlier for tip-fare ratio and visualize the coefficients below. Each curve corresponds to a variable. It shows the path of its coefficient against the L2-norm of the whole coefficient vector at as λ varies. The axis above indicates the number of nonzero coefficients at the current λ.
...
Finally after traing and testing, we are getting extremenly good results now (R^2 of `r ridge_model$dev` as compared to only 12% for tip fare ratio) for our model now.
...
The second penalized regression model that we are going to use is Lasso. Again first we plot the coefficients. Each curve corresponds to a variable. It shows the path of its coefficient against the L1-norm of the whole coefficient vector at as λ varies. The axis above indicates the number of nonzero coefficients at the current λ. We can clearly see coefficients going down to zero (instead of just shrinking) here. 
...
Then we plot the Mean Square Error cross-validation curve (red dotted line below), and upper and lower standard deviation curves along the λ sequence (error bars). Two selected λ’s are indicated by the vertical dotted lines.

```{r plot_MSE_lasso_tip, include=TRUE, echo= FALSE}
plot(cv_lamda)
```

Finally after traing and testing, we see that we are getting extremenly good results now (R^2 of `r lasso_model$dev` as compared to only 12% for tip fare ratio) for our model now.
...
We perform all the steps exactly in same manner as we did earlier for tip-fare ratio and we are getting extremely good results now (R^2 of `r elnet_model$dev`as compared to only 12% above) for Elastic Net model.

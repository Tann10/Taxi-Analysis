...
In this context, our research question is as follows: Using a subset of NYC taxi data from June 2019, what factors play a significant role in the prediction of tip fare ratio and tip amount paid via credit card? 
...
All the variables has different means and standard deviation; thus, we need to scale all the variables for our analysis
...
Model evaluation metric used for all models are r-squared and mean absolute percentage error (MAPE). MAE or MSE are scale dependent variables. We wanted to use scale independent model evaluation creteria hence we decided to use MAPE.MAPE is measured as follows:
...
We observe that p is greater than 0.05 for fit3, which implies that fit2 and fit3 are same.
...
After removal of the outliers from the data there is no improvement in the values for tip fare ratio. Below are the values for MAPE and r-squared.
...
Below is the summary from km with the first 3 features selected with Linear model.
...
As most of our variables were correlated with each other, and there were around 27 number of features, we decided to use principal component analysis (PCA) as a variable reduction technique.
...
We decided to run a simple linear regression model on those components to check if this increase the variability as indicated by r-squared.
...
We observe that p is greater than 0.05 for fit2, which means that fit1 and fit2 are same.
...
# plot actual versus perdicted values 

...
cnames<- c("fare_amount",	"pickup_hrs", "dropoff_hrs",	"trip_duration",	"trip_distance",	"DOLocationID",	"PULocationID",	"tip_fare_ratio")
...
fit0 <- lm(tip_fare_ratio ~ fare_amount, data = taxipro )
summary(fit0)
vif(fit0)
plot(fit0)

fit1 <- lm(tip_fare_ratio ~ fare_amount+trip_duration, data = taxipro )
summary(fit1)
vif(fit1)
plot(fit1)
...
fit3 <- lm(tip_fare_ratio ~ fare_amount+trip_duration+trip_distance+pickup_hrs, data = taxipro )
summary(fit3)
vif(fit3)
plot(fit3)

fit4 <- lm(tip_fare_ratio ~ fare_amount+trip_duration+trip_distance+pickup_hrs+PULocationID, data = taxipro )
summary(fit4)
vif(fit4)
plot(fit4)

fit5 <- lm(tip_fare_ratio ~ fare_amount+trip_duration+trip_distance+pickup_hrs+PULocationID+dropoff_hrs, data = taxipro )
summary(fit5)
vif(fit5)
plot(fit5)
```
...
fit6 <- lm(tip_fare_ratio ~ fare_amount+trip_duration+trip_distance+pickup_hrs+PULocationID+dropoff_hrs+DOLocationID, data = taxipro )
summary(fit6)
vif(fit6)
plot(fit6)

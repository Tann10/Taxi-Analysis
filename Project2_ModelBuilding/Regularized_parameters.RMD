...
influential_points <- function(df_out, model) {
  # set dummy value in the beginning
  influential <- dim(df_out)[1]
  
  # loop till all points are out from cooks distance
  ...
  model <- lm(eval(summary(model)$call), data = df_out)
...
cooks_distance(train, model)

df_out <- influential_points(train, model)
dim(df_out)
cooks_distance(df_out, model)
model_2 <- lm(tip_fare_ratio ~ fare_amount, data = df_out)
summary(model_2)
```
...
summary(model_3)
cooks_distance(train, model_3)

df_out <- influential_points(train, model_3)
dim(df_out)

dim(df_out)


model_3 <- lm(tip_fare_ratio ~ fare_amount, data = df_out)
cooks_distance(df_out, model_3)
df_out <- influential_points(df_out, model_3)

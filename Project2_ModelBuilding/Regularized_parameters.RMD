...
taxiprocor[c("tip_fare_ratio","tip_amount"),]
#we observe that fare_amount, trip_distance, trip_duration has best correlation with tip fare ratio and tip amount
...
#We observe that p is > 0.05 for fit3 implies fit1 and fit2 are same.

```
Since the results are not satisfactory we perform step by varification.

```{r step_by_feature_selection}
...
Let us build a linear model with fare amount to get significany results
```{r }
...
model.final.pred <- add_predictions(test,fit2_ta)
...
actuals_preds <- data.frame(cbind(actuals=model.final.pred$tip_amount, predicteds=model.final.pred$pred))  # make actuals_predicteds dataframe.
correlation_accuracy <- cor(actuals_preds)  # 91.4%
head(actuals_preds)
```
...
```{r lm-fare_amount}
#Let us select first three features selected by feature selection function above.
fit_without_fare_amount <- lm(tip_amount ~ .-fare_amount, data = taxipro )
...
<!-- # printing only the non-zero coefficients -->
<!-- tmp_coeffs <- coef(elnet_model) -->
<!-- data.frame(name = tmp_coeffs@Dimnames[[1]][tmp_coeffs@i + 1], coefficient = tmp_coeffs@x) -->

<!-- # finding significant variables i.e. calculating p-values for all coefficients -->
<!-- # library(ISLR) -->
<!-- W <- as.matrix(coef(elnet_model)) -->
<!-- W -->
...
<!-- # # extracting only the p-values of all variables -->
<!-- # a <- summary(lm(y~x))$coefficients[,"Pr(>|t|)"] -->
<!-- # a -->

<!-- library(broom) -->
<!-- tm = tidy(lm_elnet) -->
<!-- options(scipen = 999) -->
<!-- # removing 1st row i.e. intercept  -->
<!-- tm <- tm[-1, ] -->
<!-- tm -->
<!-- # get variables with p value less than 0.05 -->
<!-- sig_var_elnet <- tm$term[tm$p.value < 0.05] -->
<!-- ``` -->
...
```{r elnet}
com1_df_sub <- subset( com_df_sub, select = -tip_amount )
set.seed(123)
# We first setup our cross-validation strategy, which will be 5 fold. 
cv_5 = trainControl(method = "cv", number = 5)
# We then use train() with method = "glmnet" which is actually fitting the elastic net.
ratio_elnet = train(
  tip_fare_ratio ~ ., data = com1_df_sub,
  method = "glmnet",
  trControl = cv_5
)
...
```{r elnet}
com2_df_sub <- subset( com_df_sub, select = -tip_fare_ratio )
...
get_best_result(tip_elnet_large)
...
# Fit the final model on the training data
elnet_model <- glmnet(x, y, alpha = 0.9, lambda = 0.02313856)
# Dsiplay regression coefficients
coef(elnet_model)
...
# Positions of optimal (with respect to error) trees 
min(tip_cv_seq$size[optimal_cvtree_tip])
```
...
# Positions of optimal (with respect to error) trees 
min(ratio_cv_seq$size[optimal_cvtree_ratio])
```
...
# Plot tree
tree_ratio_cv = cv.tree(taxi_tree_ratio, best = 3)

...
```{r }
model_1 <- lm(tip_fare_ratio ~ fare_amount, data = df_sub)
summary(model_1)

model_2 <- lm(tip ~ ., data = df_sub)
summary(model_2)
```
...
tree_pred_log = predict(taxi_tree_log, newdata = test)

# Obtain MSE
tree_mse_log = (sum((tree_pred_log - test$tip_fare_ratio)^2)) / nrow(test)

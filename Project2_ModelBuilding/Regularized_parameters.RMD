...
pca_train <- data.frame("tip_fare_ratio" = splitted_dfs["y_train"]$y_train['tip_fare_ratio'], pr.out$x)[,1:7]
...
pca_train <- data.frame("tip_amount" = splitted_dfs["y_train"]$y_train['tip_amount'], pr.out$x)[,1:7]
...
ridge_model_2 <- glmnet(x, y, alpha = 0, lambda = cv_lamda$lambda.min,standardize = F)
...
# Fit the final model on the training data
elnet_model <- glmnet(x, y, alpha = 0.9, lambda = 0.02313856, standardize = FALSE)
# Dsiplay regression coefficients
coef(elnet_model)
